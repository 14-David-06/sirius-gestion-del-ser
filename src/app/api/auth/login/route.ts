...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const data = await airtableQuery(
      "Personal",
      `{Numero Documento}='${escaped}'`,
      ["ID Empleado", "Nombre completo", "Numero Documento", "Password", "Estado de actividad", "Rol"]
    );
    ...
    const storedHash = f["Password"] as string | undefined;
    if (!storedHash) {
      return NextResponse.json(
    ...
    const rolLinks = (f["Rol"] as string[]) ?? [];
    let rol = "Estándar";
    if (rolLinks.length > 0) {
      try {
        const rolRecord = await airtableGet(
          `Roles%20y%20Permisos/${rolLinks[0]}?fields[]=Nivel_Acceso`
        );
        rol = (rolRecord.fields?.Nivel_Acceso as string) ?? "Estándar";
      } catch {
    ...
    const idCore = (f["ID Empleado"] as string) ?? "";
    const nombre = (f["Nombre completo"] as string) ?? "";

    const token = await signJWT(
      { sub: record.id, idCore, cedula: String(f["Numero Documento"] ?? cedula), nombre, rol },
      JWT_SECRET
    );
