...
async function fetchRecientes(idCore: string) {
  ...
  const sortPermiso  = encodeURIComponent(FIELDS.PERMISO.FECHA_SOLICITUD);
  ...
  const [permisos, vacaciones, novedades] = await Promise.allSettled([
    fetch(`https://api.airtable.com/v0/${BASE}/${encodeURIComponent(TABLES.PERMISO)}?filterByFormula=${formula}&sort[0][field]=${sortPermiso}&sort[0][direction]=desc&maxRecords=5`, { headers: { Authorization: `Bearer ${KEY}` }, cache: "no-store" }).then((r) => r.json()),
    fetch(`https://api.airtable.com/v0/${BASE}/${encodeURIComponent(TABLES.VACACIONES)}?filterByFormula=${formula}&sort[0][field]=${sortVac}&sort[0][direction]=desc&maxRecords=5`, { headers: { Authorization: `Bearer ${KEY}` }, cache: "no-store" }).then((r) => r.json()),
    fetch(`https://api.airtable.com/v0/${BASE}/${encodeURIComponent(TABLES.NOVEDADES)}?filterByFormula=${formula}&sort[0][field]=${sortNov}&sort[0][direction]=desc&maxRecords=5`, { headers: { Authorization: `Bearer ${KEY}` }, cache: "no-store" }).then((r) => r.json()),
  ]);
  ...
  if (vacaciones.status === "fulfilled") {
    for (const r of (vacaciones.value.records ?? []) as AirtableRecord[]) {
      rows.push({
        ...
        estado:  String(r.fields[FIELDS.VACACIONES.ESTADO] ?? "Pendiente"),
      });
    }
  }
...
const ACTIONS = [
  { label: "Solicitar Permiso",    desc: "Médico, personal, calamidad y más",           href: "/dashboard/solicitudes/permiso",    color: "#1a51a8" },
  { label: "Solicitar Vacaciones", desc: "Registra tu período de descanso",              href: "/dashboard/solicitudes/vacaciones", color: "#6bb543" },
  { label: "Reportar Novedad",     desc: "Horas extra, incapacidad, cambios de horario", href: "/dashboard/solicitudes/novedades",  color: "#e07b39" },
];
...
export default async function SolicitudesPage() {
  ...
  const recientes = await fetchRecientes(payload.idCore);
